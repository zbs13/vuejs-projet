<template>
    <div class="relative">
        <input class="field" v-model="value" @input="searchUsers($event)" placeholder="Chercher un utilisateur" />
        <div v-if="this.$data.searchStatus" class="font-s-20 font-b p-10">Aucun résultat</div>
...
export default {
    ...
    methods: {
        searchUsers: async function(event) {
            let resSearchUser = await dispatchApi("users", "searchUsers", event.target.value)
            // del user existant
            let userToAdd = [];
